...
void XVideoWidget::repaintView(MediaData* data)
{
    if(data->width!=width || data->height!=height)
        return;

    memcpy(datas[0], data->data[0], width*height);
    memcpy(datas[1], data->data[1], width * height/4);
    memcpy(datas[2], data->data[2], width * height/4);
