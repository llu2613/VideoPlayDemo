...
/**
 * Simple OpenGL Qt Widget to display YUV frames using only shaders
 * https://github.com/GIPdA/yuvglwidget
 */
...
void YUVGLWidget::repaintView(MediaData *mediaData)
{
    setYPixels((uint8_t*)mediaData->data[0], mediaData->linesize[0]);
    setUPixels((uint8_t*)mediaData->data[1], mediaData->linesize[1]);
    setVPixels((uint8_t*)mediaData->data[2], mediaData->linesize[2]);
    update();
}
